...
const nodeOptions = computed<NodeOption[]>(() => {
  ...
  for (const stream of props.graph.streams) {
    options.push({
      graphId: streamNodeId(stream.id),
...
function labelForHopId(id: string): string {
  return (
    labelByGraphId.value.get(deviceNodeId(id)) ??
...
function resolvePathNode(graphId: string): LatencyPathNode | null {
  ...
  const node = (props.graph.processing_nodes ?? []).find(
...
async function measure() {
  ...
  const path = nodePath
    .map(resolvePathNode)
    .filter((entry): entry is LatencyPathNode => entry !== null);
  ...
  loading.value = true;
  try {
    result.value = await invoke<LatencyPingResult>("measure_latency_ping", {
      path,
    });
  } catch (err) {
...
<template>
  <div
    class="latency-ping-panel"
    :class="{ 'latency-ping-panel--collapsed': collapsed }"
  >
    <button
      type="button"
    ...
    >
      ...
      <span class="latency-ping-panel-toggle-icon">{{
        collapsed ? "▲" : "▼"
      }}</span>
    </button>
    ...
    <div v-if="!collapsed" class="latency-ping-panel-body">
      <p class="latency-ping-panel-hint">
        Estimated buffering latency along a signal path, from PipeWire's own
        scheduling data — not a real measured round-trip.
      </p>
      ...
      <p v-if="pathNotFound" class="latency-ping-panel-message">
        No signal path between these two nodes.
      </p>
      <p
        v-else-if="error"
        class="latency-ping-panel-message latency-ping-panel-message--error"
      >
        {{ error }}
      </p>
      ...
      <div v-else-if="result" class="latency-ping-panel-result">
        <p class="latency-ping-panel-total">
          ...
          <template v-else
            >Total: incomplete — see hop with no data below</template
          >
        </p>
        <ul class="latency-ping-panel-hops">
          <li v-for="hop in result.hops" :key="hop.id">
            <span class="latency-ping-panel-hop-label">{{
              labelForHopId(hop.id)
            }}</span>
            <span
              v-if="hop.latency_ms !== undefined"
              class="latency-ping-panel-hop-value"
            >
              {{ hop.latency_ms.toFixed(2) }}ms ({{ hop.quantum }}/{{
                hop.rate
              }})
            </span>
            <span
              v-else
              class="latency-ping-panel-hop-value latency-ping-panel-hop-value--missing"
              >no data</span
            >
          </li>
        </ul>
