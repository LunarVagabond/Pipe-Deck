 ...
 * `limiter`, and `stereo_widener` graduated to real node buttons in the
 * General category below (issues #313/#311/#314). */
...
const emit = defineEmits<{
  ...
  "copy-id": [];
  close: [];
  "add-node": [type: "output" | "input" | "fan_out" | "mixer" | "eq5band" | "delay" | "limiter" | "widener"];
...
function onPickNodeType(type: "output" | "input" | "fan_out" | "mixer" | "eq5band" | "delay" | "limiter" | "widener") {
  openCategory.value = null;
  emit("add-node", type);
}
