...
describe("computeSlotPosition", () => {
  ...
  it("falls back to the group's own position when it has no members yet", () => {
    expect(
      computeSlotPosition([], groupPosition, "row", "right", {
        width: 50,
        height: 50,
      }),
    ).toEqual(groupPosition);
  });

  it("places a left-edge row insert to the left of the leftmost member", () => {
    const members = [
      { id: "a", position: { x: 100, y: 20 }, width: 100, height: 50 },
    ];
    const position = computeSlotPosition(
      members,
      groupPosition,
      "row",
      "left",
  ...
  it("places a top-edge column insert above the topmost member", () => {
    const members = [
      { id: "a", position: { x: 30, y: 100 }, width: 100, height: 50 },
    ];
    const position = computeSlotPosition(
      members,
      groupPosition,
