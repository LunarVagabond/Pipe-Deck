...
const CONTROLS = [
  ...
  },
  {
    key: "feedbackPercent",
    label: "Feedback",
    param: "feedback_percent",
    min: 0,
    max: 100,
    unit: "%",
  },
  {
    key: "feedforwardPercent",
    label: "Feedforward",
    param: "feedforward_percent",
    min: -100,
    max: 100,
    unit: "%",
  },
...
async function onControlChange(
  ...
  const response = await invoke<{ success: boolean; message?: string }>(
    "update_processing_node_delay_params",
    {
      ...
      feedbackPercent:
        param === "feedback_percent" ? value : props.feedbackPercent,
      feedforwardPercent:
        param === "feedforward_percent" ? value : props.feedforwardPercent,
    },
  );
  if (!response.success) {
...
<template>
  <div
    class="routing-graph-node-delay nodrag"
  ...
  >
    ...
    <div
      ...
      :key="control.key"
      class="routing-graph-node-delay-row"
    >
      ...
      />
      <span class="routing-graph-node-delay-value"
        >{{ valueFor(control.key) }}{{ control.unit }}</span
      >
    </div>
  </div>
</template>
