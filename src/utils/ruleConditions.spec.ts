...
describe("inferStreamCategory", () => {
  it.each([
    ...
    expect(inferStreamCategory(makeStream({ app_name: needle }))).toBe(
      category,
    );
  });
  ...
  it("matches on executable when app_name doesn't hint at a category", () => {
    expect(
      inferStreamCategory(
        makeStream({ app_name: "MyApp", executable: "steam" }),
      ),
    ).toBe("Game");
  });
  ...
  it("matches on executable regardless of casing", () => {
    expect(
      inferStreamCategory(
        makeStream({ app_name: "MyApp", executable: "Steam.exe" }),
      ),
    ).toBe("Game");
  });
  ...
  it("returns undefined when nothing matches", () => {
    expect(
      inferStreamCategory(makeStream({ app_name: "Unknown Thing" })),
    ).toBeUndefined();
  });
});
...
describe("liveSuggestionsForType", () => {
  it("collects unique, sorted, non-empty values for the given field", () => {
    ...
    expect(liveSuggestionsForType(streams, "app_name")).toEqual([
      "Alpha",
      "Zeta",
    ]);
  });
  ...
  it("returns an empty list when no stream has the field", () => {
    expect(
      liveSuggestionsForType(
        [makeStream({ media_name: undefined })],
        "media_name",
      ),
    ).toEqual([]);
  });
...
describe("formatConditionSummary", () => {
  it("formats a regex condition with its field label and pattern", () => {
    ...
    expect(formatConditionSummary(condition)).toBe(
      "Window Class matches /Disc.*/",
    );
  });
  ...
  it("formats a direction condition as Capture or Playback", () => {
    expect(
      formatConditionSummary({ type: "direction", value: "capture" }),
    ).toBe("Capture");
    expect(
      formatConditionSummary({ type: "direction", value: "playback" }),
    ).toBe("Playback");
  });

  it("formats identity and other value-based conditions as the raw value", () => {
    expect(formatConditionSummary({ type: "identity", value: "firefox" })).toBe(
      "firefox",
    );
    expect(formatConditionSummary({ type: "category", value: "Game" })).toBe(
      "Game",
    );
  });
});
