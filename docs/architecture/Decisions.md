...
- Decision:
  ...
  - The real risk isn't a missed exclusion, it's **wrong-bucket inclusion**: several call sites (`backend/linux/pw_dump.rs`'s live-graph classification, `~line 55` and `~line 349`; `src/utils/routingLayout.ts`'s device-column/lane assignment) currently treat *any* `pipe-deck-*`-named node as a `Device`. Left unchanged, a `pipe-deck-proc-*` node would get silently absorbed into `RuntimeGraph.devices` instead of `processing_nodes` — the direct descendant of #105's "enrichment code doesn't recognize the new object" failure, just manifesting as misclassification instead of a dropped edge. `pw_dump.rs` and `routingLayout.ts` are updated to check the `pipe-deck-proc-` prefix **before** the generic `pipe-deck-` device classification and route those nodes into the new `processing_nodes` collection instead. A new `naming_allowlist_coverage` test (Rust, mirrored in a TS unit test) asserts a synthetic `pipe-deck-proc-mixer-test` name is classified as a processing node — never a `Device` — by every one of: `pw_dump.rs`, `routingLayout.ts`, `filter_chain::is_pipe_deck_device`, `virtual_devices.rs`, `virtual_ops.rs`, `config/store.rs`, `rules/matching.rs`, `graph_routing.rs`. CI fails if a new call site is added without updating this table.
