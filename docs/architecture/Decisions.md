...
- **Addendum (issue #148, day 1):** The spike was promoted into production code (`src-tauri/src/pipewire/native_host.rs`), gated behind a new `native-effects` Cargo feature (default off) plus a runtime `PIPE_DECK_NATIVE_EFFECTS=1` toggle, wired through the previously-unused `AudioBackend::load_effect_chain`/`unload_effect_chain`/`effect_chain_capabilities` methods sketched above. Restart-based (`swap_to_effect_chain`) remains the unconditional default. Two lifecycle-ownership questions this addendum resolves partially:
  - **`pw::deinit()` is deliberately never called** for the life of the process — resolves the segfault hazard above by removing the teardown-ordering race entirely, rather than getting per-call-site ordering right.
  - **Ownership placement is temporary and explicitly not what issue #148 asks for**: the native host lives in-process inside the GUI binary, not the daemon, because `daemon/mod.rs` is currently a `Type=oneshot` restore-and-exit process with no GUI↔daemon IPC. Building that, plus recovery from a daemon restart against the live PipeWire graph, is real scope deferred to a follow-up #148 session. This addendum records the "daemon-owned" requirement as **not yet satisfied** and still open. The leak-soak question above is likewise still open — today's change only exercises a handful of manual attach/detach cycles, not a real soak.
