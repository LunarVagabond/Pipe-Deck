...
- Decision:
  ...
  - A Group's wiring to its members renders as ordinary edges (`collectEdges.ts`'s existing generic per-port edge loop, unchanged for every other kind) — no node cloning. Real member devices stay exactly where they are on the canvas and remain fully independently routable elsewhere; grouping a device doesn't claim it exclusively. A per-node collapse/expand toggle (`composables/groupExpansion.ts`, plain localStorage-backed state, same tier as `groups.ts`) hides/shows a Group's own output edges; collapsed is the default so a many-member group doesn't clutter the canvas.
