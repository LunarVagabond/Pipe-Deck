...
- Primary graph topology (nodes/links) comes from a persistent native `pw::registry::Registry` connection (`backend/linux/pw_registry.rs`, #410/PD-040) — not a `pw-dump` shellout; supplemented with `pactl` for stream targets and levels (`graph_enrich.rs`), same as before. Falls back to the original `pw-dump -N`/`pw-dump -m` shellout transport (below) if the native connection can't be established at startup.
- Pipe Deck-owned virtual devices use stable `virtual-*` IDs from the in-app registry, not raw `node-*` IDs.
- Live refresh reacts to real `global`/`global_remove` registry events pushed on a continuously-running `pw::thread_loop::ThreadLoopRc` (the same lifecycle pattern `pipewire/native_host.rs` uses) instead of polling `pw-dump -m`'s stdout for "something changed" — a topology change (a stream/device appearing or disappearing) now reaches the frontend within the same coalescing window described below, but via a real event instead of a shelled-out monitor process. A periodic 1-second re-enrichment tick still runs underneath (registry events don't fire for a node's own param changes, e.g. a volume/mute change made in another app), so that data doesn't go stale between real topology events. The pre-#410 fallback (`pw-dump -m`, or a 1-second poll if that monitor process can't even be started) is kept for the case the native connection itself fails to come up.
- **Refresh coalescing policy** (#57): under high stream churn, a burst of registry events can arrive in quick succession. Rather than debouncing per-event (which still fires one full graph refresh per event, never letting the graph settle), the assembler thread waits for a 200ms quiet gap before rebuilding and pushing a single coalesced `graph-updated` — bounded by a 400ms hard cap so sustained churn still surfaces changes promptly rather than starving entirely. This keeps the UI update rate bounded while routing/mixer actions still reflect well within 500ms. (Pre-#410, this same shape of debounce/cap applied to `pw-dump -m` line events instead.)
