...
### AppImage host-library patch (issues #349 and #299)

`make build`'s AppImage step is followed by `scripts/fix-appimage-glib.sh`, which strips linuxdeploy's bundled `libglib-2.0`/`libgobject-2.0`/`libgio-2.0`/`libgmodule-2.0`, `libpipewire*.so*`, and PipeWire SPA `libspa-*.so*` files and symlinks out of the `.AppDir` before it is repackaged. The SPA namespace hyphen is intentional: unrelated near-prefix libraries such as `libspandsp` and `libspatialite` remain packaged. The GLib-family copies can be older than the host's WebKitGTK stack expects and abort GDK/EGL setup with `Could not create default EGL display: EGL_BAD_PARAMETER`. Pipe Deck must likewise use the host's PipeWire libraries and live session; a bundled copy can make system PipeWire tools or a future subprocess resolve an incompatible library through AppRun's `LD_LIBRARY_PATH`. Removing these bundled copies lets the dynamic linker fall through to the host libraries, as it already does for EGL/GL/Mesa.

The script requires `unsquashfs`/`mksquashfs` (`squashfs-tools`); if they are not installed, `make build` logs a warning and skips the patch rather than failing — CI installs `squashfs-tools` explicitly so release builds are always patched. A bundled GLib-family library must be present and removed, preserving the existing issue 349 guard. PipeWire and SPA are exclusion invariants: either family may already be absent, but each is checked independently for residue after removal. A post-processing failure stops `make build` before signing. Since the patch mutates the `.AppImage` after Tauri's own build-time updater signing, a successful `make build` re-signs it via `tauri signer sign` whenever `TAURI_SIGNING_PRIVATE_KEY`(`_PATH`) is set (as it is in CI); unsigned local dev builds just get the patch with no `.sig`.

Run `make test-appimage-postprocess` for the focused fixture check; it also runs as part of `make check`.
