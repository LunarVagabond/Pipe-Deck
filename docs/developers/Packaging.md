...
`make build`'s AppImage step is followed by `scripts/fix-appimage-glib.sh`, which strips linuxdeploy's bundled `libglib-2.0`/`libgobject-2.0`/`libgio-2.0`/`libgmodule-2.0`, `libpipewire*.so*`, and `libspa*.so*` files and symlinks out of the `.AppDir` before it is repackaged. The GLib-family copies can be older than the host's WebKitGTK stack expects and abort GDK/EGL setup with `Could not create default EGL display: EGL_BAD_PARAMETER`. Pipe Deck must likewise use the host's PipeWire libraries and live session; a bundled copy can make system PipeWire tools or a future subprocess resolve an incompatible library through AppRun's `LD_LIBRARY_PATH`. Removing these bundled copies lets the dynamic linker fall through to the host libraries, as it already does for EGL/GL/Mesa.
