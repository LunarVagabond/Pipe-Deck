...
- **A Group node is a gesture-driven Fan-out variant (issue #80, PD-035).** Ctrl-click/Cmd-click (vue-flow's multi-selection key — Shift is reserved for marquee/box-select) 2+ existing output-direction devices (hardware or virtual) already on the canvas, then right-click → "Group Selected Outputs" — wires a new node to every selected device's output in one atomic action, instead of Fan-out's connection-by-connection build-up. Functionally identical to Fan-out otherwise (one input, growable multi-target output), just created differently and with its own icon/label. Grouping a device does not remove or restrict it from being wired elsewhere — it remains fully independently routable as a normal target, and the real device node itself shows no trace of group membership. A Group behaves like a terminal/leaf node on the canvas — it draws no edges to its members at all. Instead, its members show as an inline, collapsible name list on the Group node itself, each row highlighting that member's real node on hover and offering a one-click "remove from group."
