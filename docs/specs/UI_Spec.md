...
Virtual output devices split into two distinct roles (PD-031, issue #287): a **Bus** and a terminal **Output (virtual)**. Virtual inputs are a third, unaffected kind. Concretely:

- **A Bus** groups sounds, can carry an attached effect chain (legacy per-node attachment mechanism, still supported — see "Processing Nodes" below for how this is evolving), and feeds onward to another Bus, a terminal Output, a virtual mic, or a processing node — the same mental model as a submix on a mixing desk (see "Bus"/"Submix" in `Audio_Terminology.md`). **Buses can chain into other Buses** (e.g. a "Game Audio" submix feeding a "Master Mix" bus) or into a terminal Output. The Routing graph's `targetsForVirtualSink` allows this.
- **An Output (virtual) is a true dead end**, symmetric with a physical output: apps/streams play into it, but it cannot route onward to anything (no other Bus, no Output, no mic mix), cannot host an effect, and renders with no output pin in the Routing graph — nothing can be dragged out of it. This is the new, more restrictive role; it exists alongside Bus rather than replacing what a virtual output used to do.
- **Virtual inputs stay leaves.** A virtual input (virtual mic) merges sources via `mix_sources`, but is never itself a routing source — this asymmetry is intentional, not an oversight: a virtual mic's job is to be the thing apps *consume*, not something that feeds further downstream. Only a Bus (not a terminal Output) qualifies as a mic-mix source, same "true dead end" rule as onward routing.
- **Creation is explicit, not inferred.** The "Create virtual device" dialog and the Routing graph's right-click "Add node" menu both offer three choices — Bus, Output (virtual), Input — rather than the earlier two (Output, Input). Every virtual output created before this split is treated as a Bus on upgrade, preserving existing chaining/effects behavior unchanged; the restrictive Output role is opt-in for newly created devices only.
- Under the hood, both Bus and Output (virtual) are still the same `module-null-sink` primitive — the split is a Pipe Deck-side data model and routing-permission distinction (`Device.virtual_role`), not a new PipeWire mechanism. It does **not** change how the node is exposed to the rest of the system: another app can still pick either one directly as a playback destination via PipeWire's own device list (tracked separately, issue #288).
...
- **Legacy device-attached effects (the pre-PD-032 model) still exist and are unaffected.** `Device.effect_state`/`EffectChainConfig` and the swap-by-identity attach/detach flow on a Bus device continue to work exactly as before; migrating that surface onto the new node model (or retiring it) is tracked as follow-up work, not assumed by this section. The pre-existing mic-mix mechanism (`Device.mix_sources`) is the one piece PD-032 explicitly generalizes into the Mixer Node — see PD-032 for its migration story.
