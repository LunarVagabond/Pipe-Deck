...
- **A Bus** groups sounds, can carry an effect chain (see "Effects as Attachments" below), and feeds onward to another Bus, a terminal Output, or a virtual mic — the same mental model as a submix on a mixing desk (see "Bus"/"Submix" in `Audio_Terminology.md`). **Buses can chain into other Buses** (e.g. a "Game Audio" submix feeding a "Master Mix" bus) or into a terminal Output. The Routing graph's `targetsForVirtualSink` allows this.
