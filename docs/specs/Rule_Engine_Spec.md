...
### `window_class` matching

`WindowClass` condition matches are case-insensitive, and tolerant of the
reverse-DNS `application.id` form Wayland compositors report in place of a
true X11 `WM_CLASS` — a stream reporting `org.mozilla.firefox` matches a rule
authored against `firefox`, and vice versa (the reverse-DNS prefix is
stripped from whichever side has one and compared against the other; see
`window_class_matches` / `short_window_class` in
`core/rules/matching.rs` / `core/stream_identity.rs`).
