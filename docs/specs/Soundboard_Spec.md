...
2. **#396 — Soundboard tab UI skeleton.** New sidebar view, built against placeholder/mock data first.
3. **#394 — configured folder + file listing.** A user-set folder path (persisted in `config.yaml`, see `docs/specs/Config_Spec.md`) and a command to list/validate sound files in it, wired into the tab once it exists.
