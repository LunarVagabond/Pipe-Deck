...
#[tauri::command]
pub async fn create_output_group(
    label: String,
    ...
    let mut engine = state.engine.write().await;
    let node = engine
        .create_output_group(&label, &member_device_ids)
        .map_err(|error| error.to_string())?;
    engine.emit_graph_update(&app);
    Ok(node)
}
...
#[tauri::command]
pub async fn update_processing_node_pan_params(
    node_id: String,
    balance_percent: i32,
    app: tauri::AppHandle,
    state: State<'_, AppState>,
    ...
    let mut engine = state.engine.write().await;
    let result = engine
        .update_processing_node_pan_params(&node_id, balance_percent)
        .map_err(|error| error.to_string())?;
    engine.emit_graph_update(&app);
    Ok(result)
}
